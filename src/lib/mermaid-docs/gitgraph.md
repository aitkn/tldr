...
config:
  gitGraph:
    showBranches: true          %% default: true
    showCommitLabel: true       %% default: true
    rotateCommitLabel: true     %% default: true
    mainBranchName: "main"      %% default: "main"
    mainBranchOrder: 0          %% default: 0
    parallelCommits: false      %% default: false
---
```

### Parallel Commits (v10.8.0+)

When `parallelCommits: true`, commits show at uniform distances instead of proportional to parent distance.

## Themes

Five pre-defined themes: `base`, `forest`, `dark`, `default`, `neutral`

### Theme Variables

**Branch colors:** `git0` through `git7` (cycles for 8+ branches)

**Branch labels:** `gitBranchLabel0` through `gitBranchLabel7`

**Commit labels:**
- `commitLabelColor` - text color
- `commitLabelBackground` - background
- `commitLabelFontSize` - font size

**Tags:**
- `tagLabelColor` - text color
- `tagLabelBackground` - background
- `tagLabelBorder` - border
- `tagLabelFontSize` - font size

**Highlight commits:** `gitInv0` through `gitInv7`
